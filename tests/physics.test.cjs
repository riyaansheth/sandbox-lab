...
test('dressed ragdolls are the same human: same parts, masses, joints and limits, and the simulation cannot tell them apart',()=>{
  ...
  assert.ok(require('../items.js').ITEMS.filter(i=>i.ragdoll&&i.outfit).map(i=>i.id).join()==='human1,civilian,cop,criminal,detective');
});
...
// ---- special humans
test('the immortal is hurt like anyone - wounds, breaks, lost limbs, knocked out - but never dies, and comes round in the end',()=>{
  const s=new Simulation().seed(3);s.configure({organDamage:true});const e=s.spawn('immortal',1000,555);advance(s,60);const head=e.bodies[0],chest=e.bodies[2];
  s.shoot({x:head.position.x-200,y:head.position.y},{x:head.position.x,y:head.position.y},null,{energy:30,diameter:12.7});advance(s,5);assert.ok(e.alive,'a .50 through the head');assert.equal(e.causeOfDeath,undefined);
  s.damage(chest,200,chest.position,'bullet',{x:1,y:0});s.damage(e.bodies[14],90,e.bodies[14].position,'impact',{x:1,y:0});advance(s,60*20);assert.ok(e.alive,'bled for twenty seconds');assert.ok(e.bodies.some(b=>b.plugin.wounds.length)&&e.blood>=30);
  const same=new Simulation().seed(3),man=same.spawn('human',1000,555),imm=same.spawn('immortal',1300,555);advance(same,30);for(const x of [man,imm])same.damage(x.bodies[9],40,x.bodies[9].position,'cut',{x:1,y:0});assert.equal(man.bodies[9].plugin.hp,imm.bodies[9].plugin.hp,'the same wound as anyone');
...
test('Generator: a hundred batteries - it kills a bare ragdoll at once and leaves a scattered skeleton, and only feeds Storm',()=>{
  ...
  advance(s,60);assert.ok(e.bodies.every(b=>b.plugin.char>.9),`burnt to the bone: ${e.bodies.map(b=>b.plugin.char.toFixed(2)).join(' ')}`);assert.equal(e.bodies.length,17,'still in one piece');
  const feed=new Simulation().seed(7),g3=feed.spawn('generator',900,600).bodies[0],st=feed.spawn('storm',940,555);advance(feed,60);feed.activate(g3);advance(feed,12);
  assert.ok(st.alive&&st.power>100&&st.bodies.every(b=>!(b.plugin.char>0)),`Storm only drinks it, and a generator overcharges him: ${st.power}%`);
  const off=new Simulation().seed(7),g2=off.spawn('generator',900,600).bodies[0],e2=off.spawn('human',940,555);advance(off,60);advance(off,60);assert.ok(e2.alive,'and does nothing switched off');
});
test('Storm: electricity heals his wounds, and brings him back from death',()=>{
  const s=new Simulation().seed(4);s.configure({organDamage:false});const e=s.spawn('storm',900,555);advance(s,60);const arm=e.bodies.find(b=>b.plugin.slot===10);
  s.damage(arm,40,arm.position,'cut');s.damage(arm,30,arm.position,'burn');const hurt=arm.plugin.hp,wounds=arm.plugin.wounds.length;e.blood=60;
  for(let i=0;i<4;i++)s.shock(e.bodies[2]);assert.ok(arm.plugin.hp>hurt&&e.blood>60,`hp ${hurt.toFixed(0)}->${arm.plugin.hp.toFixed(0)}, blood ${e.blood.toFixed(0)}`);assert.ok(arm.plugin.bleed===0,'the bleeding stops');
  const dead=new Simulation().seed(4),d=dead.spawn('storm',900,555);advance(dead,30);dead.kill(d,'test');assert.ok(!d.alive);dead.shock(d.bodies[2]);assert.ok(d.alive&&d.blood===100,'a shock revives him');
});
test('Storm: while he has charge he cannot die; each death he cheats costs charge, and empty he dies like anyone',()=>{
  const s=new Simulation().seed(4);const e=s.spawn('storm',900,555);advance(s,30);e.power=25;
  s.kill(e,'test');assert.ok(e.alive&&e.power===15,`cheated death: ${e.power}%`);e.blood=5;advance(s,60*5);assert.ok(e.alive&&e.blood>=30,'and cannot bleed out');
...
test('Storm: electricity charges him instead of hurting him; he spends it on lightning from the hands and beams from the eyes; empty, nothing',()=>{
  ...
  const bat=s.spawn('battery',860,620).bodies[0];s.activate(bat);advance(s,60*3);s.activate(bat);assert.ok(e.power>60,'a battery charges him further');
  const zapped=new Simulation().seed(3),z=zapped.spawn('storm',1000,555);advance(zapped,30);zapped.lightning(z.bodies[2].position.x);assert.ok(z.power>=60&&z.alive&&z.bodies.every(b=>b.plugin.hp===100),'lightning is only more power');assert.ok(z.bodies.some(b=>b.plugin.surge>0),'and he surges');
  {const q=new Simulation().seed(3);q.configure({organDamage:false});const st=q.spawn('storm',900,555),foe=q.spawn('human',1120,555);advance(q,60);st.power=100;const before=foe.bodies.reduce((n,b)=>n+b.plugin.hp,0);assert.equal(q.activate(st.bodies.find(b=>b.plugin.slot===10)),'Lightning');assert.ok(st.power<100,'a throw costs charge');assert.ok(foe.bodies.reduce((n,b)=>n+b.plugin.hp,0)<before,'and hurts the one in front of him');assert.ok(q.traces.some(t=>t.zap));
