...
(() => {
  ...
  function chargeRow(e){return e?.conduit?`<div class="stat-row"><span>Charge</span><b>${Math.round(e.power||0)}%</b></div>`:e?.immortal?'<div class="stat-row"><span>Immortal</span><b>cannot die</b></div>':'';}
  ...
  function render(){
    ...
    for(const e of sim.entities){if(!e.conduit||!e.alive||!(e.power>2))continue;const head=e.bodies.find(b=>b.plugin.slot===0);if(!head)continue;const p=head.plugin,face=p.flip?-1:1,eye=Vector.add(head.position,Vector.rotate({x:face*p.w*.25,y:-p.h*.05},head.angle)),firing=sim.time-(e.laserAt??-9)<.1,k=Math.min(1,e.power/60),r=(firing?20:8+k*10)*(0.9+.1*hash(Math.floor(sim.time*20)));
