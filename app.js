...
(() => {
  ...
  const TOOLS=[
    ...
    {id:'heal',name:'Heal',symbol:'✚',key:'8',title:'A fresh start',desc:'Restore health and extinguish an object. Broken joints stay broken.'},
    {id:'delete',name:'Delete',symbol:'⌫',key:'9',title:'Clean up',desc:'Click an object to remove it and its connections.'}
  ];
  ...
  function setTool(id){state.tool=id;state.spawn=null;state.ropeStart=null;state.rotation=0;sim.endDrag();document.querySelectorAll('.object-card').forEach(b=>b.classList.remove('active'));document.querySelectorAll('.tool').forEach(b=>{b.classList.toggle('active',b.dataset.tool===id);b.setAttribute('aria-pressed',String(b.dataset.tool===id));});const t=TOOLS.find(t=>t.id===id);$('#tool-caption-icon').textContent=t.symbol;$('#tool-caption-name').textContent=t.title;$('#tool-caption-desc').textContent=t.desc;canvas.style.cursor=id==='grab'?'grab':'crosshair';}
  function chooseSpawn(id){state.spawn=id;state.ropeStart=null;state.rotation=0;sim.endDrag();document.querySelectorAll('.object-card').forEach(b=>b.classList.toggle('active',b.dataset.object===id));document.querySelectorAll('.tool').forEach(b=>{b.classList.remove('active');b.setAttribute('aria-pressed','false');});const item=CATALOG.find(c=>c.id===id);$('#tool-caption-icon').textContent='＋';$('#tool-caption-name').textContent='Place '+item.name.toLowerCase();$('#tool-caption-desc').textContent='Click to place · Q / E to rotate · Esc to grab';canvas.style.cursor='crosshair';}
  ...
  function drawObject(c,kind,p={},time=0){
    ...
    if(kind==='gun'){c.fillStyle='#465158';c.fillRect(-12,7,12,18);c.fillStyle='#b8c3c1';c.fillRect(-w/2+3,-h/2+2,w-7,5);c.fillStyle='#252e32';c.fillRect(w/2-5,-h/2+1,5,h-2);c.fillRect(0,9,11,3);}
    if(kind==='sword'){c.fillStyle='#e4e8df';c.fillRect(-2,-h/2+2,3,h-25);c.fillStyle='#b9a172';c.fillRect(-16,h/2-24,32,5);c.fillStyle='#675e50';c.fillRect(-6,h/2-19,12,19);}
  ...
  function render(){
    ...
    for(const b of sim.bodies){const p=b.plugin;if(b.bounds.max.x<left||b.bounds.min.x>right||b.bounds.max.y<top||b.bounds.min.y>bottom)continue;
      ...
      ctx.save();ctx.translate(b.position.x,b.position.y);ctx.rotate(b.angle);drawObject(ctx,p.kind,p,sim.time);
    ...
    if(state.inside&&state.spawn){ctx.save();ctx.globalAlpha=.45;ctx.translate(state.worldPointer.x,state.worldPointer.y);ctx.rotate(state.rotation);if(state.spawn==='human'||state.spawn==='android'){for(const [part,x,y,w,h] of Sandbox.ANATOMY){ctx.save();ctx.translate(x,y);drawObject(ctx,state.spawn,{part,w,h,hp:100});ctx.restore();}}else drawObject(ctx,state.spawn);ctx.restore();}
    if(state.inside&&['blast','fire','shock','heal'].includes(state.tool)&&!state.spawn){ctx.strokeStyle=state.tool==='blast'?'#e3b07966':'#c6d9d977';ctx.lineWidth=1/camera.zoom;ctx.setLineDash([4,5]);ctx.beginPath();ctx.arc(state.worldPointer.x,state.worldPointer.y,state.tool==='blast'?175:24,0,7);ctx.stroke();ctx.setLineDash([]);}
    ctx.restore();
  ...
  function perform(point,continuous=false){const body=sim.bodyAt(point);if(state.spawn&&!continuous){const e=sim.spawn(state.spawn,point.x,point.y);if(!e){toast('Chamber is full — delete some objects first.');return;}if(state.rotation){const center={x:point.x,y:point.y};for(const b of e.bodies)Body.rotate(b,state.rotation,center);}sound('impact',.1);return;}
  ...
  function release(){state.down=false;state.pan=null;sim.endDrag();canvas.style.cursor=state.spawn||state.tool!=='grab'?'crosshair':'grab';}
  canvas.addEventListener('pointerup',release);canvas.addEventListener('pointercancel',release);canvas.addEventListener('lostpointercapture',release);canvas.addEventListener('pointerleave',()=>state.inside=false);window.addEventListener('blur',()=>{release();state.shift=false;});
  canvas.addEventListener('contextmenu',e=>e.preventDefault());canvas.addEventListener('dblclick',e=>{const b=sim.bodyAt(pointer(e));if(b&&!state.spawn&&state.tool==='grab')toast(sim.activate(b));});
  ...
  document.querySelectorAll('[data-speed]').forEach(b=>b.onclick=()=>{state.speed=Number(b.dataset.speed);document.querySelectorAll('[data-speed]').forEach(el=>el.classList.toggle('active',el===b));});
  ...
  window.addEventListener('keydown',e=>{
    if(e.key==='Shift')state.shift=true;if(help.open)return;if(['INPUT','SELECT','TEXTAREA'].includes(e.target.tagName))return;
    if(e.code==='Space'){e.preventDefault();if(!e.repeat)setPaused(!state.paused);return;}
    const t=TOOLS.find(t=>t.key===e.key);if(t){setTool(t.id);return;}
    if(e.key==='Escape'){setTool('grab');select(null);}if(e.key==='?')showHelp();
    if(e.key==='/'){e.preventDefault();$('#search').focus();}
    if(e.key==='Delete'||e.key==='Backspace'){e.preventDefault();if(state.selected){sim.removeEntity(state.selected);select(null);}}
    if(e.key.toLowerCase()==='f'&&state.selected)toast(sim.activate(state.selected));
    if(e.key.toLowerCase()==='q'||e.key.toLowerCase()==='e'){const amount=e.key.toLowerCase()==='q'?-.15:.15;if(state.spawn)state.rotation+=amount;else if(state.selected)Body.rotate(state.selected,amount);}
  });window.addEventListener('keyup',e=>{if(e.key==='Shift')state.shift=false;});
  function frame(now){const elapsed=Math.min(50,now-(lastTime||now));lastTime=now;
    ...
    if(state.down&&!state.pan&&!state.spawn&&['shoot','fire','shock'].includes(state.tool))perform(state.worldPointer,true);
