...
(function (root) {
  ...
  function jointCloth(p) { const o = outfitOf(p), slot = p.slot; if (!o) return null; if (o.suit) return o.suit.dark.base; /* the joints of a sealed suit are its under-layer, never bare skin */ if (slot === 4) return o.legs ? o.legs.base : null; /* the waist: see filler(), the shirt comes from the abdomen */ if (slot === 3 || slot === 5 || slot === 8) return o.top ? o.top.base : null; if (slot === 6 || slot === 9) return o.top && o.sleeves === 'long' ? o.top.base : null;
  ...
  root.BodyArt = {
    ...
    filler(ctx, c, a, b) { const pa = c.bodyA.plugin, pb = c.bodyB.plugin, organic = pb.material === 'flesh', burnt = Math.max(Math.min(pa.char || 0, pb.char || 0), pb.grow !== undefined ? 1 - pb.grow : 0);   /* a joint to a part that is still growing is bare bone, then raw, then skinned */ ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); const strain = clamp((Math.hypot(b.x - a.x, b.y - a.y) - 3) / ((c.plugin.breakForce || 30) - 3), 0, 1);   // 0 seated .. 1 about to tear
      if (!organic) { ctx.strokeStyle = '#596d67'; ctx.lineWidth = 6; } else if (burnt > .8) { ctx.strokeStyle = burnt > .95 ? '#b9ad92' : '#7a2429'; ctx.lineWidth = 2.2; } else { const worn = (pb.slot === 4 && jointCloth(pa)) || jointCloth(pb);   /* the waist wears the shirt, which is on the abdomen - the pelvis only has the trousers - and the trousers with no shirt */ ctx.strokeStyle = burnt > .45 || strain > .6 ? '#a8434a' : worn || (depth(pb) === 0 ? '#b48d6e' : '#d6ab88'); if (worn && depth(pb) === 0 && !(burnt > .45 || strain > .6)) { ctx.stroke(); ctx.strokeStyle = 'rgba(14,10,8,.28)'; } ctx.lineWidth = Math.min(pa.w, pb.w) * .72 * (1 - .65 * strain); }
