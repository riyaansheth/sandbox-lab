(function (root, factory) {
  ...
  const IMMORTAL_FLOOR=30,IMMORTAL_REGEN=1.5; // the immortal never has less blood than this, and gets blood and organs back at this much a second
  const CONDUIT_GAIN=40,CONDUIT_BOLT=60,SURGE_AT=25,ZAP_COST=8,ZAP_EVERY=.12,ZAP_REACH=340,ZAP_DOSE=.8,ZAP_BURN=10,LASER_COST=.5,LASER_REACH=1400,LASER_BURN=2.2,LASER_HEAT=40; // Storm: charge from one ordinary shock and from a bolt; a jump that makes him surge; what a throw of lightning costs and does; what a moment of the eye beams costs and does
  ...
  class Simulation {
    ...
    vitals(e,seconds) {
      ...
      if(human){
        ...
        if(e.immortal){e.blood=Math.min(100,Math.max(e.blood,IMMORTAL_FLOOR)+seconds*IMMORTAL_REGEN);e.oxygen=Math.min(100,(e.oxygen??100)+seconds*20);if(e.organs)for(const k in e.organs)e.organs[k]=Math.min(100,e.organs[k]+seconds*IMMORTAL_REGEN);delete e.heartStops;} /* it cannot die, so it cannot stay down for ever either: blood, air and organs come back, slowly. The wounds stay */
    ...
    kill(e,cause){if(!e.alive||e.immortal)return; /* the immortal takes the harm and not the death */e.diedAt=this.time;e.heartRate=0;e.pulse=0;e.alive=false;e.upright=false;e.causeOfDeath=cause;e.consciousness='dead';e.restTime=0;e.deadFor=0;{const chest=e.bodies.find(b=>b.plugin.slot===2);if(chest&&!chest.isStatic){const way=random()<.5?-1:1;Body.setAngularVelocity(chest,chest.angularVelocity+way*rnd(.015,.04));Body.setVelocity(chest,{x:chest.velocity.x+way*rnd(.3,.9),y:chest.velocity.y});}} // a body going limp never goes straight down: it buckles to one side
    ...
    zap(e,hand,held){if(held&&this.time-(e.zapAt??-9)<ZAP_EVERY)return '';if((e.power||0)<ZAP_COST)return held?'':'Out of charge: hit him with electricity first';e.zapAt=this.time;e.power-=ZAP_COST;
      ...
      if(best){this.shock(best,ZAP_DOSE,to);this.damage(best,ZAP_BURN,to,'burn');}else{this.burst(to.x,to.y,10,'#dff3ff',6);if(this.settings.decals)this.addStain({x:to.x,y:this.groundY-1,r:rnd(5,9),scorch:true,age:0});}this.onEffect('electric',.6);return held?'':'Lightning';}
    // Laser beams from the eyes: a straight line from the eyes the way he faces (and his head is tilted), burning the first thing it meets and setting what burns alight; held, it keeps burning, a little charge a moment.
