(function (root, factory) {
  ...
  const BLAST_HUG=4,BATTERY_EVERY=1.2,GEN_EVERY=.18,GEN_DOSE=100,GEN_CHAR=.26,GEN_BURN=55,GEN_CHAIN=140; // a generator is a hundred batteries: it fires six times a second, and what it catches is bone in a second
 // seconds between a battery's discharges: a short burst, then quiet. (The shock power is the continuous one.)
  ...
  const GARMENT_TOUCH={suit:['chest','abdomen','pelvis','upper arm','thigh'],vest:['chest','abdomen'],top:['chest','abdomen','upper arm','forearm'],pants:['pelvis','thigh','shin'],hat:['head'],mask:['head','neck'],shoes:['foot'],gloves:['hand']},GARMENT_NEEDS={suit:'chest',vest:'chest',top:'chest',pants:'pelvis',hat:'head',mask:'head',shoes:'foot',gloves:'hand'},GARMENT_ORDER=['suit','hat','vest','top','mask','pants','shoes','gloves'],REDRESS_WAIT=1.2; // the parts a garment must touch to be put on; the part a body must have to wear it; which comes off first where a part wears two; seconds before a garment just taken off can be put on again
  ...
  const ARMOURS=Object.fromEntries(ITEMS.filter(i=>i.armour).map(i=>[i.garment.outfit,{...i.armour,id:i.id,name:i.name}])),ARMOUR_ON={chest:'vest',abdomen:'vest',head:'hat'}, /* a suit is not in the table: it covers every part there is */ BLUNT_K=.12,BLUNT_PAIN=18; // armour by the outfit id it is worn as; what covers which part; the share of a stopped round's energy that still reaches the body as a blow, and the pain of it
  ...
  class Simulation {
    ...
    vitals(e,seconds) {
      ...
      if(set.slowHealing){const mend=this.mending(e);if(human){e.blood=Math.min(100,e.blood+seconds*.8*mend);if(e.organs)for(const k in e.organs)e.organs[k]=Math.min(100,e.organs[k]+seconds*.4*mend);}
        for(const b of e.bodies){const p=b.plugin;p.hp=Math.min(p.maxHp,p.hp+seconds*1.5*mend);p.bone=Math.min(100,(p.bone??100)+seconds*mend);for(const w of p.wounds||[])w.bleed=Math.max(0,(w.bleed||0)-seconds*.05*mend);p.bruise=Math.max(0,(p.bruise||0)-seconds*.02*mend);if(p.wounds?.length&&random()<seconds*.06*mend)p.wounds.shift();}}
    }
    ...
    explode(x,y,radius=175,power=1) {
      ...
      for(const b of this.bodies){const dx=b.position.x-x,dy=b.position.y-y,d=Math.hypot(dx,dy);if(d>radius)continue;const f=1-d/radius;
        ...
        const cover=b.plugin.armour?.suit,plate=cover&&cover.hp>0?ARMOURS[cover.id]:null,shield=plate?.blast&&d>BLAST_HUG?plate.blast:1; /* a charge against the plate is a charge against the man; a step away and the suit takes it */
        if(shield<1)cover.hp=Math.max(0,cover.hp-f*f*170*power*(1-shield)*plate.wear*.05);
    ...
    fly(shot,L,glow=.1) {
      ...
      for(const h of hits){const body=h.body,near=h.near,far=h.far,p=body.plugin,first=!shot.first;shot.first??=body;stop=at(near);shot.done.add(body);const gone=shot.travelled+near*SHOT_REACH;
        ...
        if(armour){const held=armour.rating*cover.hp/armour.durability;
          if(armour.glance&&glancing&&shot.E<armour.glance){cover.hp=Math.max(0,cover.hp-shot.E*armour.wear*.5);this.blunt(body,shot.E*.3,stop,direction,e,armour);turn((random()<.5?-1:1)*rnd(.3,.6),near*SHOT_REACH);shot.E*=.6;if(this.shotLog)this.shotLog.push({body,E:shot.E,use:0,through:false,armour:true,glance:true});this.hitArmour(armour,stop,.5);break;}
          if(shot.E<=held){cover.hp=Math.max(0,cover.hp-shot.E*armour.wear);this.blunt(body,shot.E,stop,direction,e,armour);if(this.shotLog)this.shotLog.push({body,E:shot.E,use:0,through:false,armour:true,stopped:true});shot.spent+=shot.E;shot.E=0;spent=true;this.hitArmour(armour,stop,.6);break;}
          cover.hp=Math.max(0,cover.hp-held*armour.wear);shot.E-=held;shot.spent+=held;rate=FLESH_E*(1+FLESH_FAST*shot.E);if(this.shotLog)this.shotLog.push({body,E:shot.E+held,use:held,through:true,armour:true});this.hitArmour(armour,stop,.4);}
    ...
    charge(e,amount,at){const chest=this.chestOf(e);if(chest)this.regenerate(chest); /* what was cut off grows back */if(!e.alive){if(!chest||!e.bodies.some(b=>b.plugin.slot===0))return;this.revive(chest);e.power=0;amount=Math.max(amount,SURGE_AT);} /* electricity brings him back, head and chest whole */
    ...
    // The beam held on one spot bores in along its line: a charred trench through the skin, then the muscle, then the bone, lengthening until it is out the far side. Through the skull, it is through the brain.
    // One 'melt' wound per spot (x,y the entry, dx,dy the beam in the part's frame, len how far in, force the dose); body.js draws it.
    melt(part,at,dir){const p=part.plugin,f=p.flip?-1:1,l=Vector.rotate(Vector.sub(at,part.position),-part.angle),ld=Vector.rotate(dir,-part.angle),x=f*l.x,y=l.y;p.wounds??=[];
      let w=p.wounds.find(o=>o.type==='melt'&&Math.hypot(o.x-x,o.y-y)<MELT_NEAR);if(!w){w={type:'melt',x,y,dx:f*ld.x,dy:ld.y,radius:2.2,force:0,depth:1,seed:random(),t:this.time,bleed:0};p.wounds.push(w);}
      w.force++;w.radius=Math.min(3.6,2.2+w.force*.02);w.depth=w.force>=MELT_MUSCLE?3:w.force>=MELT_SKIN?2:1;const span=Math.abs(w.dx)*p.w+Math.abs(w.dy)*p.h;w.len=Math.min(1,w.force/MELT_THROUGH)*span;if(w.force>=MELT_BONE)w.bone=true;
      if(w.force>=MELT_THROUGH&&!w.through){w.through=true;const e=this.getEntity(part);if(p.slot===0&&e&&e.kind==='human'){if(e.organs)e.organs.brain=0;this.kill(e,'laser through the skull');}}}
    ...
    // What a round stopped by armour sounds and looks like: plate rings and throws sparks, fabric armour takes it with a dull thud.
    // A suit with a medical layer mends the man inside it: everything the body does for itself runs this many times as fast.
    mending(e){const cover=this.chestOf(e)?.plugin.armour?.suit;const plate=cover&&cover.hp>0?ARMOURS[cover.id]:null;return plate?.heal??1;}
    insulation(body){const cover=body?.plugin.armour?.suit;const plate=cover&&cover.hp>0?ARMOURS[cover.id]:null;return plate?.shock??1;} /* what share of a shock still gets through what the part wears */
    hitArmour(armour,at,volume){if(armour?.metal){this.burst(at.x,at.y,6,'#ffe7a0',6);this.onEffect('metal',volume);this.onEffect('ricochet',volume*.5);}else{this.burst(at.x,at.y,4,'#ffe7a0',3);this.onEffect('armour',volume);}}
    blunt(body,E,at,dir,e,armour) {this.noBleed=true;this.damage(body,E*this.settings.bulletDamage*BLUNT_K*(armour?.pain??1),at,'impact',dir); /* the plate spreads the blow as well as the sting */this.noBleed=false;if(e&&e.kind==='human'&&e.alive){e.pain=Math.min(100,(e.pain||0)+E*BLUNT_PAIN*(armour?.pain??1)*this.settings.painSensitivity); /* plate takes the sting out of a hit: a few rounds are nothing, a long burst still tells */body.plugin.bruise=Math.min(1,(body.plugin.bruise||0)+E*.12);}}
    ...
    shock(body,dose=1,at=null,held=false,cap=0) { /* cap: how many bodies one discharge may chain through, when the default 30 is too few (a generator in a crowd) */ /* held: one tick of the shock power, twelve a second - a shorter chain and briefer arcs, or the screen fills with them */
    ...
    // What a generator's current does on its way through flesh: it cooks it. Every pulse chars the part and burns it deep, and once there is nothing
    // left but bone (char 1) there is a blackened skeleton where the body was, still in one piece. Storm drinks it instead; the immortal cooks and lives.
    cook(touched){if(!touched)return;
      for(const b of touched){const p=b.plugin;if(p.material!=='flesh'||!p.part)continue;const e=this.getEntity(b);if(e?.conduit)continue;
        p.char=Math.min(1,(p.char||0)+GEN_CHAR);p.heat=Math.min(700,p.heat+150);this.damage(b,GEN_BURN,b.position,'burn');
        if(p.char>=1){p.burning=false;p.bleed=0;p.wounds=[];p.stains=[];if(e&&e.alive&&(p.slot===0||p.slot===2))this.kill(e,'burnt to the bone');}}} /* ponytail: it cooks them where they stand - it never takes them apart, the joints hold */
    ...
    step(dt=1000/60) {
      ...
      for(const b of bodies){const p=b.plugin;
        ...
        if(p.active&&defs[p.kind]?.device==='generator'&&Math.floor(this.time/GEN_EVERY)!==p.lastPulse){p.lastPulse=Math.floor(this.time/GEN_EVERY);p.pulseAt=this.time;this.cook(this.shock(b,GEN_DOSE,Vector.add(b.position,Vector.rotate({x:0,y:-(p.h||0)/2},b.angle)),false,GEN_CHAIN));}
      }
