(function (root, factory) {
  ...
  class Simulation {
    constructor() {
      ...
      this.boundaries.forEach(b=>b.plugin={boundary:true});Composite.add(this.world,this.boundaries);
    ...
    ragdoll(kind,x,y) {
      ...
      const opts={collisionFilter:{group},density:robot?.0036:.0018,friction:.8,restitution:.015,frictionAir:.009};
    ...
    step(dt=1000/60) {
      ...
      // Bounded relative angles prevent knees and elbows from bending backwards.
      for(const c of this.joints){if(!c.plugin.joint||c.plugin.min===undefined)continue;const a=c.bodyA,b=c.bodyB,relative=wrap(b.angle-a.angle),error=relative-clamp(relative,c.plugin.min,c.plugin.max);if(Math.abs(error)<.005)continue;
        ...
        const correction=clamp(error,-.3,.3)*.75;Body.setAngle(a,a.angle+correction*ia/total);Body.setAngle(b,b.angle-correction*ib/total);
        const velocity=b.angularVelocity-a.angularVelocity;if(Math.sign(velocity)===Math.sign(error)){Body.setAngularVelocity(a,a.angularVelocity+velocity*.65*ia/total);Body.setAngularVelocity(b,b.angularVelocity-velocity*.65*ib/total);}
      }
