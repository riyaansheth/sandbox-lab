(function (root, factory) {
 ...
 // seconds between a battery's discharges: a short burst, then quiet. (The shock power is the continuous one.)
  ...
  class Simulation {
    ...
    fly(shot,L,glow=.1) {
      ...
      for(const h of hits){const body=h.body,near=h.near,far=h.far,p=body.plugin,first=!shot.first;shot.first??=body;stop=at(near);shot.done.add(body);const gone=shot.travelled+near*SHOT_REACH;
        ...
        if(armour){const held=armour.rating*cover.hp/armour.durability;
          ...
          if(shot.E<=held){cover.hp=Math.max(0,cover.hp-shot.E*armour.wear);this.blunt(body,shot.E,stop,direction,e,armour);if(this.shotLog)this.shotLog.push({body,E:shot.E,use:0,through:false,armour:true,stopped:true});shot.spent+=shot.E;shot.E=0;spent=true;this.hitArmour(armour,stop,.6,cover.hp/armour.durability);break;}
          cover.hp=Math.max(0,cover.hp-held*armour.wear);shot.E-=held;shot.spent+=held;rate=FLESH_E*(1+FLESH_FAST*shot.E);if(this.shotLog)this.shotLog.push({body,E:shot.E+held,use:held,through:true,armour:true});this.hitArmour(armour,stop,.4,cover.hp/armour.durability);}
    ...
    hitArmour(armour,at,volume,left=1){if(armour?.metal){this.burst(at.x,at.y,6,'#ffe7a0',6);if(left<.6)this.burst(at.x,at.y,2+Math.round((1-left)*4),'#7e868d',5); /* battered plate sheds chips of itself */this.onEffect('metal',volume);this.onEffect('ricochet',volume*.5);}else{this.burst(at.x,at.y,4,'#ffe7a0',3);this.onEffect('armour',volume);}}
