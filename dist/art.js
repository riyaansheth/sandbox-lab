...
(function (root) {
  ...
  const painters = {
    ...
    battery: { pad: 5, state: p => p.active ? 'on' : '', paint(c, p) { const x = p.w / 2, y = p.h / 2;
      ...
      live(ctx, p, time) { if (!p.active) return; const x = p.w / 2, y = p.h / 2, tick = Math.floor(time * 30), pulse = 1 - (time * 3) % 1, power = .45 + .55 * pulse * pulse, n = 7; ctx.save(); ctx.globalCompositeOperation = 'lighter'; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
