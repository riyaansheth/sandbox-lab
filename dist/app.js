...
(() => {
  ...
  const state={tool:'grab',spawn:null,selected:null,paused:false,speed:1,category:'all',sub:'all',pointer:{x:0,y:0},worldPointer:{x:0,y:0},inside:false,down:false,pan:null,ropeStart:null,ropeKind:'rope',shift:false,rotation:0,sound:false};
  const TOOLS=[
    ...
    {id:'tourniquet',name:'Tourniquet',symbol:'⊘',key:'t',title:'Tie it off',desc:'Click an arm or a leg: everything below it stops bleeding. It goes numb in 20 s and dies after 2 minutes. Click again to take it off.'},
    {id:'stitch',name:'Stitch',symbol:'✂︎',key:'k',title:'Close the wound',desc:'Click a wound: it is stitched shut for good, and a round left in it is taken out. One wound per click.'},
  ...
  function sound(type,volume=.2,kind){const set=sim.settings;if(!set.sound||!set.volume)return;try{audio??=new (window.AudioContext||window.webkitAudioContext)();if(audio.state==='suspended')audio.resume();const now=audio.currentTime,level=set.volume/60;if(type==='impact'&&now-lastImpact<.12)return;if(type==='impact')lastImpact=now;
    if(type==='electric'){if(now-(lastHit.electric||0)<.07)return;lastHit.electric=now;}
    if(type==='thud'||type==='slice'||type==='wet'||type==='crack'||type==='sizzle'||type==='cloth'||type==='ricochet'||type==='armour'||type==='metal'||type==='clink'){if(now-(lastHit[type]||0)<(type==='sizzle'?.35:.07))return;lastHit[type]=now;flesh(type,now,level*Math.min(1.4,volume));return;}
    if(type==='shot'){gunshot(kind||'gun',now,level);return;}
    if(type==='laser'){const o=audio.createOscillator(),g=audio.createGain(),f=audio.createBiquadFilter();o.type='sawtooth';o.frequency.setValueAtTime(900,now);o.frequency.exponentialRampToValueAtTime(420,now+.35);f.type='bandpass';f.frequency.value=1200;f.Q.value=3;g.gain.setValueAtTime(Math.min(.2,.1*level),now);g.gain.exponentialRampToValueAtTime(.001,now+.4);o.connect(f);f.connect(g);g.connect(audio.destination);o.start(now);o.stop(now+.42);return;}
  ...
  function flesh(type,now,level){noiseBuffer??=(()=>{const b=audio.createBuffer(1,audio.sampleRate,audio.sampleRate),d=b.getChannelData(0);for(let i=0;i<d.length;i++)d[i]=Math.random()*2-1;return b;})();
    ...
    else if(type==='metal'){tone(now,.18,1900+Math.random()*600,1400,.25);burst(now,.04,'highpass',3000,2500,1,.3);} /* a round into steel: a ping */
    else if(type==='clink'){tone(now,.07,3800+Math.random()*1500,3000,.12);tone(now+.05,.05,4600,3600,.06);} /* a brass case bouncing */
  ...
  // Gunshots. Each gun is the same four layers in its own proportions: the crack of the round leaving (a very short band of noise, higher for a fast rifle round), the blast (noise through a low-pass, longer and lower for a bigger charge),
  // the thump of the gas (a falling sine), and the room answering (a long dark tail and one echo off the far wall). A .50 is all blast and room; a submachine gun a short sharp pop; a shotgun a low boom; a crossbow no gunshot at all, only the string.
  const GUN_SOUND={gun:[3200,900,120,.16,.35,1],revolver:[2400,650,85,.26,.65,1.3],smg:[3600,1200,140,.08,.16,.8],rifle:[4400,1500,105,.11,.5,1.1],ak:[3300,950,82,.15,.55,1.2],lmg:[3000,850,76,.14,.6,1.25],minigun:[3800,1150,110,.05,.12,.65],shotgun:[1700,480,58,.32,.85,1.5],hunting:[3700,1050,70,.2,1,1.4],sniper:[2500,560,44,.38,1.7,1.9]};
  function gunshot(kind,now,level){noiseBuffer??=(()=>{const b=audio.createBuffer(1,audio.sampleRate,audio.sampleRate),d=b.getChannelData(0);for(let i=0;i<d.length;i++)d[i]=Math.random()*2-1;return b;})();
    const noise=(at,length,type,f0,f1,q,peak)=>{const src=audio.createBufferSource(),f=audio.createBiquadFilter(),g=audio.createGain();src.buffer=noiseBuffer;src.loop=true;f.type=type;f.Q.value=q;f.frequency.setValueAtTime(f0,at);f.frequency.exponentialRampToValueAtTime(f1,at+length);g.gain.setValueAtTime(Math.min(.8,peak*level),at);g.gain.exponentialRampToValueAtTime(.0008,at+length);src.connect(f);f.connect(g);g.connect(audio.destination);src.start(at,Math.random()*.9);src.stop(at+length+.02);};
    if(kind==='crossbow'){const o=audio.createOscillator(),g=audio.createGain();o.type='triangle';o.frequency.setValueAtTime(210,now);o.frequency.exponentialRampToValueAtTime(120,now+.25);g.gain.setValueAtTime(Math.min(.4,.25*level),now);g.gain.exponentialRampToValueAtTime(.001,now+.3);o.connect(g);g.connect(audio.destination);o.start(now);o.stop(now+.32);noise(now,.08,'bandpass',2500,900,1.5,.25);return;} /* the string and the bolt leaving */
    const [crack,blast,thump,length,room,loud]=GUN_SOUND[kind]||GUN_SOUND.gun,l=loud*.55;
    noise(now,.025,'bandpass',crack,crack*.7,1.4,.7*l);noise(now,length,'lowpass',blast*2.4,blast*.35,.7,.9*l);
    {const o=audio.createOscillator(),g=audio.createGain();o.type='sine';o.frequency.setValueAtTime(thump*2.2,now);o.frequency.exponentialRampToValueAtTime(thump*.6,now+length*.8);g.gain.setValueAtTime(Math.min(.8,.8*l*level),now);g.gain.exponentialRampToValueAtTime(.001,now+length);o.connect(g);g.connect(audio.destination);o.start(now);o.stop(now+length+.02);}
    noise(now+.01,room,'lowpass',900,160,.5,.18*l);noise(now+.11+room*.08,room*.6,'lowpass',700,150,.5,.08*l);}
  ...
  sim.onEffect=(type,volume,kind)=>{sound(type,volume,kind);if(type==='explosion'||type==='thunder')shake=Math.min(14,volume*8)*sim.settings.shake;};
  let shake=0;
  ...
  function woundRows(p){const rows=(p.wounds||[]).map(w=>{const age=sim.time-(w.wet??w.t??0),state=w.stitched?'stitched':w.sealed?'bandaged':w.bleed>.05?(w.artery?'spurting':w.bleed>1.5?'bleeding heavily':'bleeding'):w.type==='impact'&&!w.depth?'bruise':w.type==='burn'?((w.force??99)<15?'reddened':(w.force??99)<40?'blistered':'charred'):w.type==='shock'?'burn marks':age<Sandbox.CLOT_AT?'wet':age<Sandbox.SCAB_AT?'clotted':'scabbed';
      return `<div class="stat-row"><span>${CAUSE[w.type]||w.type}${w.hits>1?' ×'+w.hits:''}</span><b>${w.type==='burn'||w.type==='shock'?'':DEPTH[w.depth??3]+' · '}${state}${w.hit?' · hit '+HIT[w.hit]:''}</b></div>`;});
    if(p.tourniquet)rows.push(`<div class="stat-row"><span>Tourniquet</span><b>on ${Math.round(sim.time-p.tourniquet.at)} s</b></div>`);if(p.necrotic)rows.push('<div class="stat-row"><span>Limb</span><b>dead</b></div>');else if(p.numb>.3)rows.push('<div class="stat-row"><span>Limb</span><b>numb</b></div>');
    for(const r of p.lodged||[])rows.push(`<div class="stat-row"><span>Lodged round</span><b>${+r.calibre.toFixed(2)} mm</b></div>`);
    for(const end of p.severed||[])rows.push(`<div class="stat-row"><span>Stump</span><b>${end.sealed?'bandaged':end.bleed>.05?'bleeding':'closed'}</b></div>`);return rows.length?`<div class="stat-row"><span>Wounds</span><b>${rows.length}</b></div>`+rows.join(''):'';}
  function chargeRow(e){return e?.conduit?`<div class="stat-row"><span>Charge</span><b>${Math.round(e.power||0)}%${e.power>100?' overcharged':''}</b></div>`:e?.immortal?'<div class="stat-row"><span>Immortal</span><b>cannot die</b></div>':'';}
  ...
  function drawObject(c,kind,p={},time=0){
    ...
    if(p.casing){c.fillStyle=p.colour||'#c9a24a';c.fillRect(-w/2,-h/2,w,h);c.fillStyle='#6e5320';c.fillRect(-w/2,-h/2,Math.max(.8,w*.18),h);c.fillStyle='#fff3c0';c.globalAlpha=.6;c.fillRect(-w/2+w*.25,-h/2,w*.5,Math.max(.5,h*.3));c.globalAlpha=1;return;} /* a brass case, its rim, a glint */
    ...
    const dressed=Sandbox.defs[kind]?.ragdoll;if(dressed){BodyArt.preview(c,p.part,p.slot??ANATOMY_SLOT[p.part],p.w,p.h,Sandbox.defs[kind].outfit);return;}
  ...
  // Clothes are grouped by what they are worn as: the kind on the garment row, with anything that stops a round under Armour.
  const CLOTHES_SUB={top:'Tops',pants:'Trousers',hat:'Headwear',shoes:'Shoes',gloves:'Gloves',mask:'Masks'},subOf=c=>c.armour?'Armour':CLOTHES_SUB[c.garment?.kind]||'Other';
  function renderSubcategories(){const bar=$('#subcategories'),subs=state.category==='Clothes'?[...new Set(CATALOG.filter(c=>c.category==='Clothes').map(subOf))]:[];bar.hidden=!subs.length;bar.replaceChildren();if(!subs.length){state.sub='all';return;}
    if(!subs.includes(state.sub))state.sub='all';
    for(const name of ['all',...subs]){const b=document.createElement('button');b.textContent=name==='all'?'All kinds':name;b.classList.toggle('active',name===state.sub);b.onclick=()=>{state.sub=name;renderSubcategories();renderCatalog();};bar.append(b);}}
  function renderCatalog(){const query=$('#search').value.trim().toLowerCase();const list=CATALOG.filter(c=>(state.category==='all'||c.category===state.category)&&(state.sub==='all'||state.category!=='Clothes'||subOf(c)===state.sub)&&`${c.name} ${c.description}`.toLowerCase().includes(query));$('#catalog-count').textContent=list.length+' objects';$('#catalog').replaceChildren();for(const item of list){const button=document.createElement('button');button.className='object-card'+(state.spawn===item.id?' active':'');button.dataset.object=item.id;button.title=item.description;button.setAttribute('aria-label','Spawn '+item.name);const preview=document.createElement('canvas');preview.width=160;preview.height=114;preview.setAttribute('aria-hidden','true');button.appendChild(preview);const name=document.createElement('span');name.className='name';name.textContent=item.name;button.appendChild(name);if(item.id==='human'){const tag=document.createElement('span');tag.className='tag';tag.textContent='Start here';button.appendChild(tag);}button.addEventListener('click',()=>chooseSpawn(item.id));$('#catalog').appendChild(button);drawMini(preview.getContext('2d'),item.id);}if(!list.length){const p=document.createElement('p');p.className='no-results';p.textContent='No objects match your search.';$('#catalog').appendChild(p);}}
  for(const tool of TOOLS){const b=document.createElement('button');b.className='tool'+(tool.id==='grab'?' active':'');b.dataset.tool=tool.id;if(tool.id==='rope')b.dataset.menu='rope';b.title=`${tool.title} (${tool.key})`;b.setAttribute('aria-label',tool.title);b.setAttribute('aria-pressed',String(tool.id==='grab'));b.innerHTML=`<span class="shortcut">${tool.key}</span><span class="symbol">${tool.symbol}</span><span class="tool-name">${tool.name}</span>`+(tool.id==='rope'?'<span class="caret">▾</span>':'');b.addEventListener('click',()=>{setTool(tool.id);if(tool.id==='rope')ropeMenu(b);});$('#tools').appendChild(b);}
  // The Rope tool holds two kinds of tie: plain rope, and wire that carries current from whatever is shocked at one end to everything on the other.
  const ROPE_KINDS=[{id:'rope',name:'Rope',desc:'A plain tie. Holds two things together.'},{id:'wire',name:'Electrical wire',desc:'Carries current from end to end, however far.'}];
  function ropeMenu(button){const open=document.querySelector('.tool-menu');if(open){open.remove();if(open.dataset.for==='rope')return;}
    const menu=document.createElement('div');menu.className='tool-menu';menu.dataset.for='rope';
    for(const kind of ROPE_KINDS){const b=document.createElement('button');b.className=kind.id===state.ropeKind?'active':'';b.innerHTML=`${kind.name}<small>${kind.desc}</small>`;b.onclick=e=>{e.stopPropagation();state.ropeKind=kind.id;state.ropeStart=null;ropeName();menu.remove();};menu.append(b);}
    button.append(menu);const close=e=>{if(!menu.contains(e.target)&&e.target!==button){menu.remove();document.removeEventListener('pointerdown',close);}};setTimeout(()=>document.addEventListener('pointerdown',close),0);}
  function ropeName(){const kind=ROPE_KINDS.find(k=>k.id===state.ropeKind),b=document.querySelector('[data-menu="rope"]');if(!b)return;b.querySelector('.tool-name').textContent=kind.name==='Electrical wire'?'Wire':kind.name;if(state.tool==='rope'){$('#tool-caption-name').textContent=kind.name;$('#tool-caption-desc').textContent=kind.desc;}}
  ...
  const REMAINS={view:0,livor:0,pale:0,face:'dead',faceId:13,gaze:0,dead:true,noGore:false,time:0,char:0,breath:0}; // parts whose owner is gone
  ...
  function render(){
    ...
    const set=sim.settings;looks.clear();for(const e of sim.entities){if(e.kind!=='human')continue;const face=!set.faces?'neutral':!e.alive?'dead':e.consciousness==='unconscious'?'closed':e.shoutT>0?'shout':(sim.time-(e.hitTime??-9)<.6||e.pain>65)?'tense':e.consciousness==='dazed'?'dazed':'neutral';
      const iris=e.immortal?'gold':e.conduit?(e.power>2&&e.alive?'storm':'stormDim'):null,irisN=iris==='gold'?1:iris==='storm'?2:iris==='stormDim'?3:0;
      looks.set(e.id,{iris,view:state.view,organs:e.organs,livor:e.alive||e.diedAt===undefined?0:clamp((sim.time-e.diedAt-LIVOR_AFTER)/LIVOR_TIME,0,1),pale:Math.max(e.blood<75?clamp((75-e.blood)/50,0,1):0,e.alive||e.diedAt===undefined?0:clamp((sim.time-e.diedAt)/LIVOR_AFTER,0,1)*.5),face,faceId:BodyArt.FACES.indexOf(face)*3+((set.faces&&e.alive?e.gaze:0)||0)+1+irisN*1000,gaze:(set.faces&&e.alive?e.gaze:0)||0,dead:!e.alive,noGore:set.noGore,time:sim.time,char:0,breath:e.alive&&set.breathing?Math.sin((e.breath||0)*Math.PI*2)*(.018+Math.min(.03,(e.pain||0)/2500)):0});}
    ...
    if(set.decals)for(const st of sim.stains){if(st.x+st.r<left||st.x-st.r>right)continue;
      if(st.hole){ctx.fillStyle='#16191b';ctx.beginPath();ctx.arc(st.x,st.y,1.6,0,7);ctx.fill();ctx.strokeStyle='#6c7275';ctx.lineWidth=.6;ctx.beginPath();ctx.arc(st.x,st.y,2.6,0,7);ctx.stroke();continue;} /* a pock in the floor or a wall */
    ...
    fillers.clear();for(const c of sim.joints){if(c.plugin.joint){fillers.set(c.bodyB,c);continue;}if(c.plugin.pierce||c.plugin.hold)continue;const a=Constraint.pointAWorld(c),b=Constraint.pointBWorld(c),wire=c.plugin.wire,live=wire&&sim.time-(c.plugin.liveAt??-9)<.25;
      ctx.strokeStyle=wire?'#2e3a42':'#c8b889';ctx.lineWidth=wire?2.5:2;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();
      if(live){ctx.save();ctx.globalCompositeOperation='lighter';ctx.strokeStyle='rgba(120,200,255,.5)';ctx.lineWidth=5;ctx.stroke();ctx.strokeStyle='rgba(240,250,255,.95)';ctx.lineWidth=1.2;ctx.stroke();ctx.restore();} /* current in it: the insulation glows from inside */
      ctx.fillStyle=wire?(live?'#dff3ff':'#b4762f'):'#d0c6aa';for(const p of [a,b]){ctx.beginPath();ctx.arc(p.x,p.y,3,0,7);ctx.fill();}}
    ...
    for(const b of drawList){const p=b.plugin;if(b.bounds.max.x<left||b.bounds.min.x>right||b.bounds.max.y<top||b.bounds.min.y>bottom)continue;
      ...
      if(p.part&&p.material==='flesh'&&set.decals&&!set.noGore&&(p.wounds?.length||p.severed?.length)){let open=false; /* runs of blood down the skin from each wound, stopping at the body's edge */
        for(const list of [p.wounds,p.severed])if(list)for(const w of list){if(!(w.run>1)||w.sealed)continue;if(!open){open=true;ctx.save();if(p.flip)ctx.scale(-1,1);BodyArt.trace(ctx,p);ctx.clip();ctx.lineCap='round';}
          const wet=clamp(1-(sim.time-(w.wet??w.t??0))/30,0,1),ex=w.x+Math.cos(w.runDir)*w.run,ey=w.y+Math.sin(w.runDir)*w.run;ctx.globalAlpha=.85;ctx.strokeStyle=ctx.fillStyle=stainColor(wet);ctx.lineWidth=1.3;ctx.beginPath();ctx.moveTo(w.x,w.y);ctx.lineTo(ex,ey);ctx.stroke();if(wet>.3){ctx.beginPath();ctx.arc(ex,ey,1.1,0,7);ctx.fill();}}
        if(open){ctx.lineCap='butt';ctx.globalAlpha=1;ctx.restore();}}
      if(p.holes?.length&&set.decals){ctx.save();if(p.flip)ctx.scale(-1,1);for(const h of p.holes){ /* bullet holes: a bright dent in metal, a splintered hole in wood, a chipped pock in stone, a star in glass */
          if(h.kind==='dent'){ctx.fillStyle='#e8ecee';ctx.beginPath();ctx.arc(h.x,h.y,2.2,0,7);ctx.fill();ctx.fillStyle='#3b4246';ctx.beginPath();ctx.arc(h.x,h.y,1.1,0,7);ctx.fill();}
          else if(h.kind==='crack'){ctx.strokeStyle='#f4fbff';ctx.lineWidth=.5;ctx.beginPath();for(let k=0;k<6;k++){const an=k*1.05+hash(h.x+k),l=3+hash(h.y+k)*4;ctx.moveTo(h.x,h.y);ctx.lineTo(h.x+Math.cos(an)*l,h.y+Math.sin(an)*l);}ctx.stroke();ctx.fillStyle='#1a1f22';ctx.beginPath();ctx.arc(h.x,h.y,.9,0,7);ctx.fill();}
          else if(h.kind==='chip'){ctx.fillStyle='#8a8d8f';ctx.beginPath();ctx.arc(h.x,h.y,2.4,0,7);ctx.fill();ctx.fillStyle='#2d3033';ctx.beginPath();ctx.arc(h.x,h.y,1.2,0,7);ctx.fill();}
          else{ctx.fillStyle='#1b1410';ctx.beginPath();ctx.arc(h.x,h.y,h.exit?1.8:1.2,0,7);ctx.fill();ctx.strokeStyle='#e0c28e';ctx.lineWidth=.5;ctx.beginPath();for(let k=0;k<4;k++){const an=k*1.57+hash(h.x*2+k)*.8;ctx.moveTo(h.x+Math.cos(an)*1.2,h.y+Math.sin(an)*1.2);ctx.lineTo(h.x+Math.cos(an)*3,h.y+Math.sin(an)*3);}ctx.stroke();}}ctx.restore();}
    ...
    for(const f of sim.flashes){const t=1-f.life/f.maxLife;if(f.muzzle){ctx.globalCompositeOperation='lighter';ctx.globalAlpha=1-t;ctx.drawImage(glowSprite('muzzle','255,210,130',.9),f.x-f.radius,f.y-f.radius,f.radius*2,f.radius*2);ctx.globalAlpha=1;ctx.globalCompositeOperation='source-over';continue;} /* a muzzle flash: a hot bloom, no ring */if(f.grow||f.surge){const tint=f.grow?'150,255,205':'130,235,255';for(const lag of f.surge?[]:[0]){ // the graft surge has no ring: just the core flash, arcs and sparks
    ...
    for(const t of sim.traces){if(t.laser){const k=t.life/t.maxLife;ctx.globalCompositeOperation='lighter';ctx.lineCap='round';ctx.strokeStyle=`rgba(255,40,30,${.35*k})`;ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(t.from.x,t.from.y);ctx.lineTo(t.to.x,t.to.y);ctx.stroke();ctx.strokeStyle=`rgba(255,120,100,${.8*k})`;ctx.lineWidth=1.8;ctx.stroke();ctx.strokeStyle=`rgba(255,245,240,${k})`;ctx.lineWidth=.7;ctx.stroke();ctx.lineCap='butt';ctx.globalCompositeOperation='source-over';continue;} /* an eye beam: a red glow, a hot core */
      if(t.bolt)continue;if(t.electric){drawElectric(t);continue;}if(!set.tracers)continue;ctx.globalAlpha=t.life/t.maxLife;ctx.strokeStyle='#f0cd84';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(t.from.x,t.from.y);ctx.lineTo(t.to.x,t.to.y);ctx.stroke();}ctx.globalAlpha=1;
    ...
    for(const e of sim.entities){if(!e.conduit||!e.alive||!(e.power>2))continue;const head=e.bodies.find(b=>b.plugin.slot===0);if(!head)continue;const p=head.plugin,face=p.flip?-1:1,eye=Vector.add(head.position,Vector.rotate({x:face*p.w*.25,y:-p.h*.05},head.angle)),firing=sim.time-(e.laserAt??-9)<.1,over=Math.min(1,Math.max(0,e.power-100)/400),k=Math.min(1,e.power/60),r=(firing?20:8+k*10+over*9)*(0.9+.1*hash(Math.floor(sim.time*20)));
      ctx.globalCompositeOperation='lighter';ctx.globalAlpha=firing?1:.35+.65*k;ctx.drawImage(glowSprite(firing?'eyeRed':'eyeBlue',firing?'255,70,50':'120,220,255',.9),eye.x-r,eye.y-r,r*2,r*2);ctx.globalAlpha=1;ctx.globalCompositeOperation='source-over';} /* his eyes glow with the charge he carries, red when they fire */
  ...
  function perform(point,continuous=false){const body=sim.bodyAt(point);
    switch(state.tool){case'grab':if(!continuous){select(body);if(body){sim.beginDrag(body,point);canvas.style.cursor='grabbing';}}break;
      case'rope':if(!continuous){if(!state.ropeStart){state.ropeStart={body,point:{...point}};toast(state.ropeKind==='wire'?'Choose the other end of the wire.':'Choose the other end of the rope.');}else{const a=state.ropeStart;if(!a.body&&!body){toast('At least one end must attach to an object.');}else{const c=sim.rope(a.body,body,a.point,point,state.ropeKind==='wire');if(c)spawned.push({rope:c});toast(state.ropeKind==='wire'?'Wire connected':'Rope connected');}state.ropeStart=null;}}break;
      ...
      case'tourniquet':if(body&&!continuous){toast(sim.tourniquet(body));select(body);}break;
      case'stitch':if(body&&!continuous){toast(sim.stitch(body,point));select(body);}break;
  ...
  window.addEventListener('keydown',e=>{
    ...
    else if(key==='x'){state.view=((state.view||0)+1)%3;toast(['Skin','X-ray: muscle layer, organs and fractures','X-ray: skeleton, organs and fractures'][state.view]);}
