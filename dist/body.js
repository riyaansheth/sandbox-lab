...
(function (root) {
  ...
  function paintPart(canvas, p, state) {
    ...
    layer(c, 1, size, s => { outline(s, part, w, h); if (swell > .05) { s.moveTo(w / 2 + 1.8 * swell, h * .04); s.ellipse(0, h * .04, w / 2 + 1.8 * swell, Math.min(h * .2, 7), 0, 0, 7); } drawSkin(s, part, w, h, slot, state);
  ...
  function sprite(body, state) {
    ...
    const clean = p.grow === undefined && !(state.livor > .05) && !p.wounds?.length && !p.severed?.length && (p.hp ?? 100) >= 48 && (p.bone ?? 100) > 50 && !(p.bruise > .05) && !(state.char > .03);
