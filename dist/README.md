...
- Wounds age: fresh ones are bright and wet, after 30 s they darken and clot, after a few minutes a dark scab closes them. Bruises come up a few seconds after the blow, spread, and go from red to purple to yellow-green before fading
...
`npm test` runs 105 behaviour tests: physics, damage, blood, organs, muscles, reactions, mobility, awareness, settings, save/restore. `npm run build` packages the standalone application in `dist`.
